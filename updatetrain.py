...
def main():
    ...
    if os.path.exists(checkpoint_path):
        checkpoint = load_checkpoint(checkpoint_path, neural_model)
        print(f"  ✓ Loaded trained weights from epoch {checkpoint.get('epoch', '?')}")
    else:
        print(f"  ⚠ No trained model found, using random weights")
        print(f"    Train a model first: python training/train.py --config configs/quick_config.yaml")
